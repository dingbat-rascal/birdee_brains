...
function M.load_dictionary(settings)
    ...
    if not has_question then
        error("Question column '" .. settings.question_column .. "' not found in CSV. Available columns: " .. table.concat(headers, ", "))
    end
    if not has_answer then
        error("Answer column '" .. settings.answer_column .. "' not found in CSV. Available columns: " .. table.concat(headers, ", "))
    end
