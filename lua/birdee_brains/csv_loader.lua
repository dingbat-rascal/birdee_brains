...
function M.scan_directory(directory)
    ...
    -- Find subdirectories
    local dir_pattern = normalized_dir .. "/*"
    local all_paths = vim.fn.glob(dir_pattern, false, true)
    ...
    if #dirs > 0 then
        debug_print("Found subdirectories:", table.concat(dirs, ", "))
    end

    return files, dirs, found_dir
end
...
--- Scan a directory for CSV files and return a list of filenames (legacy function)
--- @param directory string|nil Optional directory path to scan
--- @return table files Array of CSV filenames
--- @return string|nil found_dir The directory that was used
function M.scan_csv_files(directory)
    local files, _, found_dir = M.scan_directory(directory)
